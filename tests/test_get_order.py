...
class TestGetOrderUser:
    ...
    def test_get_order_user_with_auth(self, create_and_delete_user, valid_ingredients):
        ...
        with allure.step("Создание заказа для пользователя"):
            ...
            create_response = OrderAPI.create_order(token, valid_ingredients)
            assert create_response.status_code == 200, \
                f"Не удалось создать заказ. Статус: {create_response.status_code}, Ответ: {create_response.text}"
            
            allure.attach(
                f"Создан заказ с ингредиентами: {valid_ingredients}", 
        ...
        with allure.step("Проверка успешного получения заказов"):
            ...
            allure.attach(
                f"Найден заказ №{order.get('number')} со статусом {order.get('status')}", 
                name="Результат", 
                attachment_type=allure.attachment_type.TEXT
            )
    ...
    def test_get_order_user_without_auth(self):
        ...
        with allure.step("Попытка получения заказов без токена"):
            response = OrderAPI.get_user_orders_without_auth()
        ...
        with allure.step("Проверка ответа с ошибкой"):
            ...
            allure.attach(
                f"Ошибка: {response_data.get('message')}", 
                name="Результат", 
                attachment_type=allure.attachment_type.TEXT
            )
    ...
    def test_get_order_user_invalid_token(self):
        ...
        with allure.step("Попытка получения заказов с неверным токеном"):
            response = OrderAPI.get_user_orders(invalid_token)
    ...
    def test_get_order_user_empty_orders(self, create_and_delete_user):
        ...
        with allure.step("Проверка ответа"):
            ...
            assert len(response_data['orders']) == 0, \
                f"Ожидался пустой список, получено {len(response_data['orders'])} заказов"
            
            allure.attach(
                "У пользователя нет заказов (ожидаемое поведение)", 
                name="Результат", 
                attachment_type=allure.attachment_type.TEXT
            )